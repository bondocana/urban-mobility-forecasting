...
Download the 2025 Yellow Taxi monthly Parquet files from the NYC TLC website and place them in:
...
The individual scripts can also be run from the project root:

```powershell
python src/materialize_processed_dataset.py
python src/run_mllib_models.py --full
python src/run_pipeline_model.py --full
python src/run_udf_tuning.py --full
python src/run_tensorflow_model.py --epochs 2
python src/run_streaming_simulation.py
```

## Notes

The streaming part uses a Spark micro-batch simulation. On my local Windows setup, native Spark Structured Streaming checkpoints require extra Hadoop Windows utilities, so the simulation keeps the same streaming idea while staying easier to run locally.
